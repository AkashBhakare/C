...
int main() {
    ...
    strncpy(p1.name, "anil", sizeof (p1.name));
    strncpy(p1.ad.last_deg,
            "Master in computer Science", sizeof (p1.ad.last_deg));
    p1.ad.yr_of_passing = 2003;
    p1.ad.score = 78.89f;

    printf("\nName : %s\n", p1.name);
    if (p1.info_type == PERSONAL_DET) {
        printf("Age : %d\n", p1.pd.age);
        printf("Weight %.2f", p1.pd.weight);
        printf("Name : %s\n", p1.pd.address);
    } else {
        printf("Degree : %s\n", p1.ad.last_deg);
